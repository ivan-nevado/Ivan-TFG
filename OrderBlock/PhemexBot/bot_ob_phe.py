...
def is_ob_filled(ob, df):
    ob_type, ob_top, ob_bottom, ob_mit_index, ob_index = ob
    for i in range(ob_index + 1, len(df)):
        if (ob_type == 'bullish' and df['Low'].iloc[i] < ob_bottom) or \
           (ob_type == 'bearish' and df['High'].iloc[i] > ob_top):
...
def bot():
    ...
    for ob in reversed(list(obs.itertuples())):
        ...
        if ob_mit_index is not None and not np.isnan(ob_mit_index):  # Check if ob_mit_index is valid
            mit_index_top = df['High'].iloc[int(ob_mit_index)]  # Ensure it's an integer
            print(f'OB already mitigated, ob index: {ob_index}, with top: {ob_top}, the mit index top: {mit_index_top}')
            continue
            
        # Place a BUY order for the most recent bullish OB
        if ob_type == 1 and current_price > ob_top and macro_trend_bullish and ob_bottom > ema_value:
        ...
        # Place a SELL order for the most recent bearish OB
        elif ob_type == -1 and current_price < ob_bottom and not macro_trend_bullish and ob_top < ema_value:
            ...
            print(f'No valid conditions to place an order, current ob top:{ob_top}, ob type: {ob_type}')
