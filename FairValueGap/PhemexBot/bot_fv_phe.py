...
First it will look in at the EMA of 100 periods to see if we are bearish or bullish
...
ema_period = 100  # EMA period, can be adjusted based on optimization results
...
def bot():
    ...
    for fvg in reversed(fvgs):
        fvg_index, fvg_type, fvg_bottom, fvg_top, fvg_mit_index = fvg
         # Skip if FVG is already filled 
        if is_fvg_filled(fvg, df):
            continue

        # Place a BUY order for the most recent bullish FVG
        if fvg_type == 'bullish' and current_price > fvg_top and macro_tendency_bullish and fvg_bottom > ema_value:
        ...
        elif fvg_type == 'bearish' and current_price < fvg_bottom and not macro_tendency_bullish and fvg_top < ema_value:
